              ...
              <div class="row">
                <div class="col-12 col-md-4 col-lg-3">
                  ...
                  <ul id="filter-menu" class="list-unstyled list-group list-group-flush">
                    ...
                    $categories = get_terms('category', array('taxonomy' => 'brochure_category'));
                    foreach ($categories as $category) {
                        echo '<button type="button" class="filter-item list-group-item list-group-item-action text-secondary" data-filter="' . $category->slug . '">' . $category->name . '</button>';
                    }
                    ?>
                  </ul>
                </div>
                    ...
                    $query = new WP_Query($args);
                      if ($query->have_posts()) :
                          while ($query->have_posts()) :
                              $query->the_post();
                              $categories = get_the_terms(get_the_ID(), 'brochure_category');
                              $data_id = '';
                              if ($categories) {
                                  foreach ($categories as $category) {
                        ...
                        <div class="card border-0 bg-transparent rounded-4">
                            <div class="card-image position-relative rounded-4">
                                <a href="<?php echo esc_url(get_the_post_thumbnail_url(get_the_ID())); ?>" class="text-decoration-none text-black" data-fancybox="gallery" data-id="<?php echo esc_attr($data_id); ?>" data-caption="<?php esc_attr(the_title()); ?>">
                                    <img src="<?php echo esc_url(get_the_post_thumbnail_url(get_the_ID())); ?>" alt="<?php esc_attr($brochure_featured_image_alt); ?>" class="rounded-4">
                                </a>
                            </div>
                        </div>
                        <?php
